...
void Model_ChangeCurFile(PMODELDATA pSelf, const wchar_t *const chosenFile)
{
	...
	HANDLE hFile;
	if ((hFile = getOpeningFile(chosenFile)) != INVALID_HANDLE_VALUE) {
...
LRESULT Model_WriteFile(PMODELDATA pSelf, HANDLE hFile, BOOL saveSelected)
{
	if ((pSelf->dataSize == 0)
...
HANDLE getSavingFile(wchar_t *fileName)
{
...
void Model_SaveFileAs(PMODELDATA pSelf, BOOL saveSelected, wchar_t *const chosenFile)
{
	...
	LRESULT res;
	HANDLE hFile;
	if ((hFile = getSavingFile(chosenFile)) != INVALID_HANDLE_VALUE) {
		res = Model_WriteFile(pSelf, hFile, saveSelected);
		switch (res) {
		case 0:
			//all good
			if (saveSelected) {
				...
				pSelf->curFile = hFile;
				pSelf->isChanged = FALSE;
				//TODO: MainWindow_UpdateView(pSelf->mainView, pSelf, curFileNameChange, NULL);
			}
			break;
		case -2:
...
{
	// TODO: could be improved by writing only new waveformat chunk and changed pieces to the file
	LRESULT res = Model_WriteFile(pSelf, pSelf->curFile, FALSE);
	switch (res) {
	case 0:
		pSelf->isChanged = FALSE;
		break;
	...
	default:
		//other cases (when GetLastError returned nonzero or written less)
		MainWindow_ShowModalError(pSelf->mainView, L"При записи файла произошла ошибка", L"Ошибка", MB_ICONERROR);
		// TODO: should propose save file as or something
		break;
	}
}
...
void Model_DeletePiece(PMODELDATA pSelf)
{
	if (pSelf->curFile != INVALID_HANDLE_VALUE) {
		...
		// deletePiece sets the new selected range properly
...
{
	if ((pSelf->curFile != INVALID_HANDLE_VALUE) && (pSelf->rgSelectedRange.nFirstSample != pSelf->rgSelectedRange.nLastSample)) {
		MainWindow_UpdateView(pSelf->mainView, pSelf, playbackStop, NULL);
...
{
	if (pSelf->curFile != INVALID_HANDLE_VALUE) {
		if (!updateOnlyLastSample) {
			pSelf->rgSelectedRange.nFirstSample = range->nFirstSample;
		...
		if (pSelf->rgSelectedRange.nLastSample < pSelf->rgSelectedRange.nFirstSample) {
			sampleIndex tmp = pSelf->rgSelectedRange.nFirstSample;
